...
{       
        ...
        <br/><br/>
        <a href='http://localhost/student/password-change.php?token=$token&email=$get_email'>Click this!</a>
        ";
...
if(isset($_POST['reset_btn']))
{
    ...
    $check_email = "SELECT * FROM tbl_admin WHERE email = '$email' LIMIT 1";
    ...
    if(mysqli_num_rows($res) > 0)
    {
        ...
        $update_token = "UPDATE tbl_admin SET verify_token = '$token' WHERE email = '$get_email' LIMIT 1";
    ...
    if(!empty($token))
    {
        if(!empty($email) && !empty($password) && !empty($confirm))
        {
            ...
            if(mysqli_num_rows($check_token_res) > 0)
            {
                if($password == $confirm)
                {
                    $update_password = "UPDATE tbl_admin SET password = '$password' WHERE verify_token = '$token' LIMIT 1";
