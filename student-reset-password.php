...
{       
        ...
        $email_template = "
        <h2>You have registered as Student</h2>
        <h5>You are receiving this email because we received a password reset request for your account!</h5>
        <br/><br/>
        <a href='http://localhost/student/student-password-change.php?token=$token&email=$get_email'>Click this!</a>
        ";
...
if(isset($_POST['reset_btn']))
{
    ...
    $check_email = "SELECT * FROM tbl_student WHERE email = '$email' LIMIT 1";
    ...
    if(mysqli_num_rows($res) > 0)
    {
        ...
        $update_token = "UPDATE tbl_student SET verify_token = '$token' WHERE email = '$get_email' LIMIT 1";
        ...
        if($res2 == true)
        {
            ...
            header("Location: student-forgotpass.php");
            exit(0);
        }
        else
        {
...
if(isset($_POST['updatepass_btn']))
{
    ...
    if(!empty($token))
    {
        if(!empty($email) && !empty($password) && !empty($confirm))
        {
            ...
            $check_token = "SELECT verify_token FROM tbl_student WHERE verify_token = '$token' LIMIT 1";
            ...
            if(mysqli_num_rows($check_token_res) > 0)
            {
                if($password == $confirm)
                {
                    $update_password = "UPDATE tbl_student SET password = '$password' WHERE verify_token = '$token' LIMIT 1";
                    ...
                    else
                    {
                        ...
                        header("Location: student-password-change.php?token=$token&email=$email");
                        exit(0);
                    }
                }
                else
    ...
    else
    {
        ...
        header("Location: student-password-change.php");
        exit(0);
    }
}
